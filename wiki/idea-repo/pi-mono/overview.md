...
pi-mono/
├── packages/
│   ├── ai/           # @mariozechner/pi-ai      (~28K 行)
│   ├── agent/        # @mariozechner/pi-agent-core  (~1.7K 行)
│   ├── coding-agent/ # @mariozechner/pi-coding-agent  (~23K 行)
│   ├── tui/          # @mariozechner/pi-tui      (~11K 行)
│   └── web-ui/       # @mariozechner/pi-web-ui   (~14K 行, 新增)
```
...
**2. @mariozechner/pi-agent-core** — 通用代理运行时
...
**4. @mariozechner/pi-tui** — 差分渲染 TUI 框架
- 差分渲染终端 UI（只更新变化的部分）
- 编辑器组件（支持撤销、自动补全）
- Markdown 渲染组件
- 覆盖层（Overlay）系统

**5. @mariozechner/pi-web-ui** — 浏览器 UI 组件（新增）
