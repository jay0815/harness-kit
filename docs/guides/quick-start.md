...
在 PI 框架内运行，接入 PI 的 ExtensionAPI。当前兼容路径会注入 workflow prompt、记录 telemetry，并通过 `sendUserMessage` 反馈校验失败；目标路径是由 harness-kit 的 phase scheduler/state machine 管理 phase 边界。需要 PI 环境。
