...
export function resolveConfig(args: ParsedArgs): HarnessAgentSessionConfig {
  ...
  if (!providers.includes(args.provider as KnownProvider)) {
    throw new Error(`Unknown provider "${args.provider}". Available: ${providers.join(", ")}`);
  }

  const models = getModels(args.provider as KnownProvider);
  const model = models.find((m) => m.id === args.model) as Model<Api> | undefined;
  if (!model) {
    const modelIds = models.map((m) => m.id);
    throw new Error(
      `Unknown model "${args.model}" for provider "${args.provider}". Available: ${modelIds.join(", ")}`,
    );
  }
