...
import type { AgentMiddleware, RuntimeState, LLMResponse, AgentToolCall, AgentTool, AgentToolResult } from "./types.js";
...
describe("MiddlewarePipeline", () => {
  it("executes beforeModel in priority order", async () => {
    ...
    pipeline.register({
      ...
      name: "eval",
      beforeModel: async () => { order.push(2); },
    });
    pipeline.register({
      priority: PRIORITY_GUARD,
      name: "guard",
      beforeModel: async () => { order.push(1); },
    });
  ...
  it("chains afterModel responses", async () => {
    ...
    expect(result.stopReason).toBe("modified_a_b");
  });
  ...
  it("first beforeTool block wins", async () => {
    const pipeline = new MiddlewarePipeline();
    const block1: AgentToolResult<any> = { content: [{ type: "text", text: "block1" }], details: null };
    const block2: AgentToolResult<any> = { content: [{ type: "text", text: "block2" }], details: null };
  ...
  it("afterTool chains results", async () => {
    ...
    const result = await pipeline.runAfterTool(makeState(), makeToolCall(), undefined, makeToolResult());
  ...
  it("empty pipeline does nothing", async () => {
    ...
    const finalResult = await pipeline.runAfterTool(state, makeToolCall(), undefined, makeToolResult());
